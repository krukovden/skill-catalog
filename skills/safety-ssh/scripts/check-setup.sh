...
#
# Usage: check-setup.sh <alias>
set -u

ALIAS="${1:-}"
[ -n "$ALIAS" ] || { echo "usage: check-setup.sh <alias>" >&2; exit 2; }
...
# 2. Alias is actually defined in ~/.ssh/config. `ssh -G` always succeeds and
#    echoes the alias back as hostname when no Host block matched, so an unchanged
#    hostname means "not defined".
resolved_host=$(ssh -G "$ALIAS" 2>/dev/null | awk 'tolower($1)=="hostname"{print $2; exit}')
if [ -n "$resolved_host" ] && [ "$resolved_host" != "$ALIAS" ]; then
  ok "alias '$ALIAS' is defined in ~/.ssh/config"
else
  bad "alias '$ALIAS' is not defined in ~/.ssh/config (a human must add a Host block)"
fi

# 3. Alias is approved in the allowlist.
if [ -f "$ALLOWLIST" ] && grep -qxF "$ALIAS" "$ALLOWLIST"; then
  ok "alias '$ALIAS' is in the allowlist"
else
  bad "alias '$ALIAS' is NOT in the allowlist ('$ALLOWLIST')"
fi
...
keyok=0
for f in $(ssh -G "$ALIAS" 2>/dev/null | awk 'tolower($1)=="identityfile"{print $2}'); do
  expanded=$(printf '%s' "$f" | sed "s|^~|$HOME|")
  [ -f "$expanded" ] && keyok=1
done
if [ "$keyok" -eq 1 ]; then
  ok "a configured private key file exists for '$ALIAS'"
else
  bad "no existing private key file is configured for '$ALIAS' (run setup-host)"
fi
...
if ssh -o BatchMode=yes -o ConnectTimeout=10 -o IdentitiesOnly=yes \
       -o PreferredAuthentications=publickey "$ALIAS" true >/dev/null 2>&1; then
  ok "key-based connection to '$ALIAS' works (BatchMode, no prompt)"
else
  bad "could not connect to '$ALIAS' with key auth — check the key is authorized on the host"
fi
